...
import { Check, ChevronsUpDown, UserRound, Building2, Loader2 } from "lucide-react";
...
export function ProfileSwitcher({
  ...
  const switchProfile = (nextProfileId: number) => {
    ...
    // Un solo aviso que pasa de "cambiando" a "listo", en vez de dos apilados.
    const noticeId = toast.loading("Cambiando de perfil", `Pasando a ${next.name}...`);

    startTransition(async () => {
      try {
        const result = await setActiveProfile(nextProfileId);
        if (result && result.success === false) {
          toast.update(noticeId, "error", "No se pudo cambiar de perfil", result.error);
          return;
        }

        router.refresh();
        toast.update(noticeId, "success", "Perfil cambiado", `Ahora trabajas como ${next.name}.`);
      } catch {
        toast.update(noticeId, "error", "No se pudo cambiar de perfil", "Revisa tu conexion e intenta de nuevo.");
      }
    });
  };
    ...
    <div className="px-4 pb-4">
      <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-800 dark:bg-slate-950/60">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          {isPending ? (
            <span className="inline-flex shrink-0 items-center gap-1 rounded-md bg-blue-50 px-1.5 py-0.5 text-[10px] font-bold text-blue-600 dark:bg-blue-900/30 dark:text-blue-300">
